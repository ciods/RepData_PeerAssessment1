...
> **Mean** -- `r I(format(steps_mean,scientific=FALSE))`  
> **Median** -- `r I(format(steps_median,scientific=FALSE))`  
...
```
The 5-minute interval containing the *maximum* number of steps on average across all the days is: **`r I(max_int)`**.
...
na_rows <- sum(na_flag)
```
Total number of *missing* values in the dataset: **`r I(na_rows)`**
